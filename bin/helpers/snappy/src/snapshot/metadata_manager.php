...
class metadata_manager {
    ...
    /** @return metadata_provider_interface[] */ private function discover(): array { $p=[]; if(class_exists(summary_metadata_provider::class)){ $p[] = new summary_metadata_provider(); } return $p; }
    ...
    public function generate(string $uid): void {
        ...
        foreach($this->providers as $provider){
            try { $out=$provider->generate($uid,$dir,$ctx); if(!is_array($out)) continue; foreach($out as $item){ $name=(string)($item['name']??''); $content=(string)($item['content']??''); if($name==='') continue; $safe=preg_replace('/[^A-Za-z0-9._-]/','_',$name); if($safe==='') continue; $target=$metaDir.'/'.$safe; $tmp=$target.'.tmp'.bin2hex(random_bytes(3)); file_put_contents($tmp,$content); @rename($tmp,$target); } } catch(\Throwable $e){ /* swallow */ }
        }
    }
}
