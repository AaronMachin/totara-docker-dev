...
class share extends base_command {
    ...
    public function description(): string { return 'Share a local snapshot by ensuring a background host is running and printing a pull command'; }
    public function usage(): string { return "Usage: tsnap share <uid|prefix> [--bucket=NAME --region=REG --port=8000 --key=K --secret=S | --anon] [--prefix=P] [--endpoint=URL] [--timeout=SEC]\nEnsures background host is running (starting if needed) then prints encoded remote and a pull command for the snapshot."; }
    public function examples(): array { return ['tsnap share 7fa12c3','tsnap share --anon 7fa12c3','tsnap share --endpoint=https://example.ngrok-free.app 7fa12c3','tsnap share --bucket=mybucket --region=us-east-1 --port=9000 7fa12c3']; }
    ...
    public function run(array $args, context $ctx): int {
        ...
        if ($manager->isRunning()) { $state = $manager->state(); if (!$state) { $state = $manager->start($opts, true); $newlyStarted = true; } }
        else { try { $state = $manager->start($opts); $newlyStarted = true; } catch (RuntimeException $e) { fwrite(STDERR,'failed to start host: '.$e->getMessage()."\n"); return 2; } }
