...
class share_share_create extends base_command {
    ...
    public function usage(): string { return 'Usage: tsnap share create <snapshot-uid|prefix> [--listen=HOST:PORT|:PORT] [--ttl=SECONDS] [--max=N] [--multi] [--no-auto-export] [--lan-only]\nStarts a minimal HTTP server exposing the snapshot export artifact at /artifact. By default will attempt ngrok TCP tunnel for internet sharing unless --lan-only specified.'; }
    ...
    public function run(array $args, context $ctx): int {
        $uid=''; $listen=':0'; $ttl=300; $max=1; $multi=false; $noAuto=false; $lanOnly=false;
        foreach ($args as $a) {
        ...
        try {
            $res = $svc->serve($uid,[ 'listen'=>$listen,'ttl'=>$ttl,'max'=>$max,'multi'=>$multi,'no_auto_export'=>$noAuto,'lan_only'=>$lanOnly,'config'=>$ctx->config ], function(array $payload) use ($ctx){
                ...
                $ctx->out->json(['encoded'=>$encoded,'verification_code'=>$payload['code'],'tunnel'=>$payload['tunnel']??'none']);
            });
