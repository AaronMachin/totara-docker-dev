...
class remote implements command {
    ...
    public function description(): string {
        return 'Manage snapshot remotes (list, add, remove)';
    }
    ...
    private function add(array $args, context $ctx): int {
        ...
        foreach ($args as $arg) {
            ...
            if (str_starts_with($arg, '--endpoint=')) {
                $config['endpoint'] = substr($arg, 11);
            } elseif (str_starts_with($arg, '--bucket=')) {
                $config['bucket'] = substr($arg, 9);
            } elseif (str_starts_with($arg, '--region=')) {
                $config['region'] = substr($arg, 9);
            } elseif (str_starts_with($arg, '--key=')) {
                $config['key'] = substr($arg, 6);
            } elseif (str_starts_with($arg, '--secret=')) {
                $config['secret'] = substr($arg, 9);
            } elseif ($arg === '--path-style') {
    ...
    private function remove(array $args, context $ctx): int {
        $name = $args[0] ?? '';
        if ($name === '') {
            fwrite(STDERR, "remote remove requires <name>\n");
            return 1;
        }
        try {
