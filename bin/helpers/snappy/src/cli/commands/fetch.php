...
class fetch implements command {
    ...
    public function description(): string { return 'Fetch remote data'; }
    public function run(array $args, context $ctx): int {
        $remote = 'local'; $limit = 100;
        foreach ($args as $arg) {
        ...
        try { $rows = $ctx->manager->list($remote, false, $limit); }
        catch (\Throwable $e) { fwrite(STDERR, 'fetch failed: ' . $e->getMessage() . "\n"); return 3; }
