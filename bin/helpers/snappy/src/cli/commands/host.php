...
class host extends base_command {
    ...
    public function description(): string { return 'Manage background ngrok host: host [start|status|logs|stop] [options]'; }
    public function usage(): string { return "Usage: tsnap host [start|status|logs|stop] [--bucket=NAME --region=REG --port=8000 --key=K --secret=S | --anon] [--prefix=P] [--endpoint=URL] [--timeout=SEC]\nStarts or manages background host (ngrok or provided endpoint). 'start' is default."; }
    public function examples(): array { return ['tsnap host','tsnap host status','tsnap host logs','tsnap host stop','tsnap host --bucket=mybucket --region=us-east-1 --port=9000 --anon']; }

    public function run(array $args, context $ctx): int {
        $sub = 'start'; $recognized = ['start','status','logs','stop'];
        ...
        return match($sub) {
            ...
            default => $this->doStart($manager, $args, $ctx),
        };
    }
    ...
    private function doStop(manager $manager): int {
        ...
        $manager->stop(); echo "host: stopped\n"; return 0;
    }

    private function doLogs(manager $manager): int {
        if (!$manager->isRunning()) { echo "host: not running (start first)\n"; return 1; }
        $manager->streamLogs(); return 0; // blocks until interrupted
    }

    private function doStart(manager $manager, array $args, context $ctx): int {
        [$opts, $snapshot, $help, $error] = options::parse($args, false); // help handled by dispatcher already
        if ($error) { fwrite(STDERR, $error."\n"); return 1; }
        if ($err = $opts->normalize()) { fwrite(STDERR, $err."\n"); return 1; }
        try { $state = $manager->ensureRunning($opts); }
        catch (RuntimeException $e) { fwrite(STDERR,'start failed: '.$e->getMessage()."\n"); return 2; }
